...
func runPublish(ctx context.Context, imageNameListFromArgs []string) error {
	...
	stagesStorage, err := common.GetMetaStorage(ctx, containerBackend, &commonCmdData, common.GetMetaStorageOpts{
	...
	imagesRepoStorages, err := common.GetOptionalImagesRepoStorages(ctx, containerBackend, &commonCmdData)
	if err != nil {
		return err
	}
	...
	if len(imagesRepoStorages) > 0 {
		bundleRepo = imagesRepoStorages[0].Address()
	} else {
