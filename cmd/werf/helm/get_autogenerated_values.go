...
func runGetServiceValues() error {
	...
	if err := common.InitGiterminismInspector(&getAutogeneratedValuedCmdData); err != nil {
		return err
	}
	...
	werfConfig, err := common.GetRequiredWerfConfig(ctx, projectDir, &getAutogeneratedValuedCmdData, giterminismManager, common.GetWerfConfigOptions(&getAutogeneratedValuedCmdData, false))
	if err != nil {
	...
	if *getAutogeneratedValuedCmdData.StubTags {
		...
		conveyorWithRetry := build.NewConveyorWithRetryWrapper(werfConfig, giterminismManager, localGitRepo, []string{}, projectDir, projectTmpDir, ssh_agent.SSHAuthSock, containerRuntime, storageManager, storageLockManager, common.GetConveyorOptions(&getAutogeneratedValuedCmdData))
