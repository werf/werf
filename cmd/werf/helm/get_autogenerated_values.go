...
func runGetServiceValues(ctx context.Context, imagesToProcess build.ImagesToProcess) error {
	...
	if *getAutogeneratedValuedCmdData.StubTags {
		...
		conveyorWithRetry := build.NewConveyorWithRetryWrapper(werfConfig, giterminismManager, giterminismManager.ProjectDir(), projectTmpDir, ssh_agent.SSHAuthSock, containerBackend, storageManager, storageLockManager, common.GetConveyorOptions(&getAutogeneratedValuedCmdData, imagesToProcess))
