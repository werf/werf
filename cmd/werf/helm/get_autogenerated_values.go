...
These values includes project name, docker images ids and other`),
	...
	common.SetupRepoOptions(&getAutogeneratedValuedCmdData, cmd)
...
func runGetServiceValues(ctx context.Context) error {
	...
	if *getAutogeneratedValuedCmdData.StubTags {
		...
		stagesStorage, err := common.GetStagesStorage(stagesStorageAddress, containerBackend, &getAutogeneratedValuedCmdData)
		if err != nil {
			return err
		}
		finalStagesStorage, err := common.GetOptionalFinalStagesStorage(containerBackend, &getAutogeneratedValuedCmdData)
		if err != nil {
			return err
		}
		...
		secondaryStagesStorageList, err := common.GetSecondaryStagesStorageList(stagesStorage, containerBackend, &getAutogeneratedValuedCmdData)
		if err != nil {
			return err
		}
		cacheStagesStorageList, err := common.GetCacheStagesStorageList(containerBackend, &getAutogeneratedValuedCmdData)
		if err != nil {
			return err
		}
		...
		conveyorWithRetry := build.NewConveyorWithRetryWrapper(werfConfig, giterminismManager, []string{}, giterminismManager.ProjectDir(), projectTmpDir, ssh_agent.SSHAuthSock, containerBackend, storageManager, storageLockManager, common.GetConveyorOptions(&getAutogeneratedValuedCmdData))
