...
These values includes project name, docker images ids and other`),
	...
	common.SetupDisableDeterminism(&getAutogeneratedValuedCmdData, cmd)
	common.SetupNonStrictDeterminismInspection(&getAutogeneratedValuedCmdData, cmd)
...
func runGetServiceValues() error {
	...
	if err := determinism_inspector.Init(determinism_inspector.InspectionOptions{NonStrict: *getAutogeneratedValuedCmdData.NonStrictDeterminismInspection}); err != nil {
		return err
	}
	...
	projectDir, err := common.GetProjectDir(&getAutogeneratedValuedCmdData)
	if err != nil {
	...
	werfConfig, err := common.GetRequiredWerfConfig(ctx, projectDir, &getAutogeneratedValuedCmdData, localGitRepo, config.WerfConfigOptions{DisableDeterminism: *getAutogeneratedValuedCmdData.DisableDeterminism, Env: *getAutogeneratedValuedCmdData.Environment})
	if err != nil {
	...
	if err := ssh_agent.Init(ctx, *getAutogeneratedValuedCmdData.SSHKeys); err != nil {
	...
	if *getAutogeneratedValuedCmdData.StubTags {
		...
		conveyorWithRetry := build.NewConveyorWithRetryWrapper(werfConfig, localGitRepo, []string{}, projectDir, projectTmpDir, ssh_agent.SSHAuthSock, containerRuntime, storageManager, storageLockManager, common.GetConveyorOptions(&getAutogeneratedValuedCmdData))
