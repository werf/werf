...
func SetupWerfChartParams(cmd *cobra.Command, commonCmdData *cmd_werf_common.CmdData) {
	...
	cmd_werf_common.SetupDisableGitermenism(commonCmdData, cmd)
...
func InitWerfChartParams(ctx context.Context, commonCmdData *cmd_werf_common.CmdData, wc *werf_chart.WerfChart, chartDir string) error {
	...
	if m, err := deploy.GetSafeSecretManager(ctx, chartDir, chartDir, *commonCmdData.SecretValues, wc.LocalGitRepo, *commonCmdData.DisableGitermenism, *commonCmdData.IgnoreSecretKey); err != nil {
		return err
	} else {
		wc.SecretsManager = m
	}
