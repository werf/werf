...
func SetupRepoOptions(cmdData *CmdData, cmd *cobra.Command) {
	SetupInsecureRegistry(cmdData, cmd)
	SetupSkipTlsVerifyRegistry(cmdData, cmd)
	SetupRepo(cmdData, cmd)
}

func SetupRepo(cmdData *CmdData, cmd *cobra.Command) {
	cmdData.Repo = NewRepoData("repo", RepoDataOptions{})
...
func GetStagesStorageAddress(cmdData *CmdData) (string, error) {
	...
	return *cmdData.Repo.Address, nil
}
