...
func SetupNetworkParallelism(cmdData *CmdData, cmd *cobra.Command) {
	...
	if val, err := util.GetIntEnvVar("WERF_NETWORK_PARALLELISM"); err != nil {
		graceful.Panic(fmt.Sprintf("bad WERF_NETWORK_PARALLELISM value: %s", err), 1)
	} else if val != nil {
		defVal = int(*val)
	} else {
...
func SetupKubeQpsLimit(cmdData *CmdData, cmd *cobra.Command) {
	...
	if val, err := util.GetIntEnvVar("WERF_KUBE_QPS_LIMIT"); err != nil {
		graceful.Panic(fmt.Sprintf("bad WERF_KUBE_QPS_LIMIT value: %s", err), 1)
	} else if val != nil {
		defVal = int(*val)
	} else {
...
func SetupKubeBurstLimit(cmdData *CmdData, cmd *cobra.Command) {
	...
	if val, err := util.GetIntEnvVar("WERF_KUBE_BURST_LIMIT"); err != nil {
		graceful.Panic(fmt.Sprintf("bad WERF_KUBE_BURST_LIMIT value: %s", err), 1)
	} else if val != nil {
		defVal = int(*val)
	} else {
...
func GetNetworkParallelism(cmdData *CmdData) int {
	if *cmdData.NetworkParallelism < 1 {
		graceful.Panic(fmt.Sprintf("bad network parallelism value: %d (should be >= 1)", *cmdData.NetworkParallelism), 1)
	}
...
func GetDeployGraphPath(cmdData *CmdData) string {
	...
	default:
		graceful.Panic(fmt.Sprintf("invalid --deploy-graph-path %q: extension must be either .dot or unspecified", *cmdData.DeployGraphPath), 1)
		return ""
	}
}
...
func GetRollbackGraphPath(cmdData *CmdData) string {
	...
	default:
		graceful.Panic(fmt.Sprintf("invalid --rollback-graph-path %q: extension must be either .dot or unspecified", *cmdData.RollbackGraphPath), 1)
		return ""
	}
}
...
func SetupReleasesHistoryMax(cmdData *CmdData, cmd *cobra.Command) {
	...
	if err != nil {
		graceful.Panic(fmt.Sprintf("bad WERF_RELEASES_HISTORY_MAX value: %s", err), 1)
	}
...
func GetIntEnvVarStrict(varName string) *int64 {
	...
	if err != nil {
		graceful.Panic(fmt.Sprintf("bad %s value: %s", varName, err), 1)
	}
	return valP
}
