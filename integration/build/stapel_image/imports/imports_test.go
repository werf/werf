...
var _ = Describe("Stapel imports", func() {
	...
	Context("importing files and directories from artifact", func() {
		...
		It("should allow importing files and directories, optionally rename files and directories and merge directories", func() {
			...
			Expect(werfRunOutput("imports_app1-003", "cat", "/usr/local/FILE")).To(Equal("FILE\n"))
			Expect(werfRunOutput("imports_app1-003", "cat", "/usr/locallll")).To(Equal("FILE\n"))
			Expect(werfRunOutput("imports_app1-003", "cat", "/usr/newlocal/file")).To(Equal("GOGOGO\n"))
			Expect(werfRunOutput("imports_app1-003", "cat", "/usr/newlocal/a/b/FILE")).To(Equal("FILE\n"))
			Expect(werfRunOutput("imports_app1-003", "cat", "/usr/share/file")).To(Equal("GOGOGO\n"))
			Expect(werfRunOutput("imports_app1-003", "cat", "/usr/share/a/b/FILE")).To(Equal("FILE\n"))
			Expect(werfRunOutput("imports_app1-003", "ls", "/usr/share/apk")).To(Equal("keys\n"))
			Expect(werfRunOutput("imports_app1-003", "cat", "/file2")).To(Equal("GOGOGO\n"))
			Expect(werfRunOutput("imports_app1-003", "cat", "/file")).To(Equal("GOGOGO\n"))
		})
	})
})
