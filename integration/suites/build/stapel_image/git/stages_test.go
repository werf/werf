...
func checkResultedFilesChecksum(ctx context.Context) {
	...
	docker.RunSucceedContainerCommandWithStapel(nil, SuiteData.WerfBinPath, SuiteData.TestDirPath, []string{fmt.Sprintf("-v %s:%s", SuiteData.TestDirPath, containerTestDirPath)}, []string{diffCommand})
}
