...
Данная статья содержит описание аннотаций, которые меняют поведение механизма отслеживания ресурсов в процессе выката с помощью werf. Все аннотации должны быть объявлены в шаблонах чарта.
 ...
 - [`werf.io/resource-policy`](#политика-ресурса) — защищает ресурс от создания, изменения или удаления.
...
Пересоздания и удаления, вызванные аннотациями [`werf.io/delete-policy`](#политика-удаления-ресурса) или `helm.sh/hook-delete-policy`, также пропускаются значениями `skip-recreate` и `skip-delete`.
...
По умолчанию для обычных ресурсов политика не задана, а для Namespace релиза используется `skip-delete`. Значение аннотации `helm.sh/resource-policy` преобразуется в значение `werf.io/resource-policy`, но `werf.io/resource-policy` имеет приоритет.
