...
Данная статья содержит описание аннотаций, которые меняют поведение механизма отслеживания ресурсов в процессе выката с помощью werf. Все аннотации должны быть объявлены в шаблонах чарта.
 ...
 - [`werf.io/resource-policy`](#политика-ресурса) — определяет, какие операции werf может выполнять с ресурсом: создание, обновление, пересоздание, удаление.
 ...
 - [`werf.io/ignore-readiness-probe-fails-for-CONTAINER_NAME`](#игнорирование-неудачных-readiness-проб-контейнера) — переопределить высчитываемый автоматически период, в течение которого неуспешные readiness-пробы будут игнорироваться и не будут переводить ресурс в состояние ошибки.
 ...
 - [`werf.io/show-logs-only-for-number-of-replicas`](#логи-для-указанного-числа-реплик) — включить логирование вывода только для указанного числа реплик ресурса.
 - [`werf.io/show-logs-only-for-containers`](#логи-для-указанных-контейнеров) — включить логирование вывода только для указанных контейнеров ресурса.
 - [`werf.io/show-service-messages`](#отображение-служебных-сообщений) — включить вывод сервисных сообщений и событий Kubernetes для данного ресурса.
...
Можно указать несколько значений одновременно. Пересоздания, вызванные аннотациями [`werf.io/delete-policy`](#политика-удаления-ресурса) или `helm.sh/hook-delete-policy`, пропускаются значением `skip-recreate`, а вызванные ими удаления — значением `skip-delete`.
...
По умолчанию для обычных ресурсов политика ресурса не задана. Для Namespace релиза `skip-delete` устанавливается всегда — в дополнение к политике, заданной на нём. Аннотация `helm.sh/resource-policy: keep` преобразуется в `werf.io/resource-policy: skip-delete`. Если задана `werf.io/resource-policy`, то `helm.sh/resource-policy` игнорируется полностью — значения не объединяются.
