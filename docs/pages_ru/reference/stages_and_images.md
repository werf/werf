...
Следующая таблица иллюстрирует зависимости в Dockerfile-образе, Stapel-образе и [Stapel-артефакте]({{ site.baseurl }}/configuration/stapel_artifact.html).
...
Рекомендуется использовать docker registry в качестве хранилища стадий. Werf по умолчанию использует этот режим [при работе в CI/CD системах]({{ site.baseurl }}/reference/plugging_into_cicd/overview.html).
...
Данная сигнатура используется [при тегировании образов по содержимому]({{ site.baseurl }}/reference/publish_process.html#тегирование-образов-по-содержимому) и для импорта файлов из артефактов и образов (сигнатура стадий артефакта или образа влияет на сигнатуру стадии импортов целевого образа, куда происходит импорт).

## Образы

_Образ_ — это **готовый к использованию** Docker-образ, относящийся к опеределенному состоянию приложения в соответствии со [стратегией тегирования]({{ site.baseurl }}/reference/publish_process.html).
...
_Стадии_, в отличие от конечных _образов_ не предназначены для прямого использования. Основное отличие между конечными образами и стадиями — разное поведение [политики очистки]({{ site.baseurl }}/reference/cleaning_process.html#очистка-по-политикам) по отношению к ним, я также различия в структуре хранимой мета-информации.
...
На текущий момент, _образы_ создаются только в [_процессе публикации_]({{ site.baseurl }}/reference/publish_process.html) (publish) и хранятся в _репозитории образов_.
...
См. также статью про [переключение на распределённый режим работы]({{ site.baseurl }}/guides/switch_to_distributed_mode.html).

## Дальнейшее руководство

Больше информации о процессе сборки для stapel и Dockerfile сборщиков [доступно в статье]({{ site.baseurl }}/reference/build_process.html).
