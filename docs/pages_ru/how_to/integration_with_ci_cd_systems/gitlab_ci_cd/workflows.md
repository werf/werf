...
Если необходимо выполнить авторизацию с произвольными учётными данными, `docker login` должен выполняться после вызова `werf ci-env` (подробнее про авторизацию [в отдельной статье]({{ "usage/cleanup/supported_container_registries.html#авторизация" | true_relative_url }})).
...
Для выполнения `werf dismiss` требуется werf.yaml, так как в нём содержаться [шаблоны имени релиза и namespace]({{ "/usage/deploy/releases/naming.html" | true_relative_url }}). При удалении ветки нет возможности использовать исходники из git, поэтому в задании `Stop Review` используется werf.yaml, сохранённый при выполнении задания `Review`, и отключено подтягивание изменений из git (`GIT_STRATEGY: none`).
...
Более подробно ознакомиться с возможностями очистки, встроенными в werf, можно [здесь]({{ "usage/cleanup/cleanup.html" | true_relative_url }}).
