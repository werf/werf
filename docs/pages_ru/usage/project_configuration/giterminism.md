...
Использование секретов из переменных окружения и файлов усложняет совместное использование и воспроизводимость конфигурации в заданиях CI и среди разработчиков.

Функция `allowEnvVariables` разрешает использование определённых переменных окружения в качестве секретов.
Чтобы разрешить использование значений секретов из файлов необходимо использовать функцию `allowFiles` где необхоимо указать относительный или абсолютный путь до файла, который содержит секрет.
Чтобы разрешить использование произволных значений секретов типа `value` вы должны указать их идентификаторы (`id`) в функции `allowValueIds`.
Для активации `allowEnvVariables`, `allowFiles` и `allowValueIds` необходимо использовать [werf-giterminism.yaml]({{ "reference/werf_giterminism_yaml.html" | true_relative_url }}), но мы рекомендуем еще раз подумать о возможных последствиях.
