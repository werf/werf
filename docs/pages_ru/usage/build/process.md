...
Указать `--meta-repo` проекту, у которого в `--repo` уже есть метаданные, можно сразу: новые метаданные начинают писаться в отдельный репозиторий, а werf v2 при этом может параллельно продолжать работать с тем же `--repo`. Но метаданные, уже накопленные в `--repo`, туда не переезжают, и werf, читающий только `--meta-repo`, их не видит — поэтому перенесите их до первой очистки, иначе `cleanup` будет принимать решения о судьбе стадий по неполной картине и удалит используемые образы:

```shell
werf meta-repo migrate --repo registry.mycompany.org/project --meta-repo registry.mycompany.org/project-meta
```

`migrate` копирует метаданные в `--meta-repo` (сначала копирование с проверкой, идемпотентно — можно запускать повторно), записывает маркер в `--repo`, после чего удаляет каждый оригинал из `--repo` — но только после того, как его копия подтверждена в `--meta-repo`. Чтобы сохранить оригиналы, укажите `--remove-source=false`.
