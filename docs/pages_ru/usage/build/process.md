...
`migrate` копирует четыре семейства метаданных в `--meta-repo` (сначала копирование с проверкой, идемпотентно — можно запускать повторно), после чего записывает маркер в `--repo`. Флаг `--remove-source` удаляет оригиналы из `--repo`, но только после того, как каждая копия подтверждена в `--meta-repo`.
