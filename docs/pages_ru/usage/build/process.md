...
Одновременная работа werf v2 и werf v3 с одним `--repo` на время перехода требует `--remove-source=false` и имеет свои ограничения — см. [Миграцию с v2 на v3]({{ "/resources/migration_from_v2_to_v3.html" | true_relative_url }}).
