...
Флаг `--save-build-report` поддерживают команды: `werf build`, `werf converge`, `werf plan`, `werf render`, `werf lint`, `werf bundle publish`, `werf helm get-autogenerated-values`, `werf stages copy`.
...
Где `<IMAGE>` — имя образа в верхнем регистре, в котором символы `/`, `-`, `.` заменены на `_`. Для неименованных образов префикс — `WERF_`.
...
Флаг `--use-build-report` поддерживают команды: `werf converge`, `werf plan`, `werf render`, `werf export`, `werf run`, `werf kube-run`, `werf lint`, `werf compose`, `werf bundle publish`, `werf helm get-autogenerated-values`, `werf stages copy`.
