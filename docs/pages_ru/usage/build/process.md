...
- пропуск `--meta-repo` или указание другого значения — **жёсткая ошибка**;
...
`migrate` копирует четыре семейства метаданных в `--meta-repo` (сначала копирование с проверкой, идемпотентно — можно запускать повторно), после чего записывает маркер в `--repo`. Флаг `--remove-source` удаляет оригиналы из `--repo`, но только после того, как каждая копия подтверждена в `--meta-repo`. Если `--repo` разделяется несколькими проектами ещё с тех времён, когда werf ещё не проставлял метки на эти записи, непомеченные устаревшие записи других проектов могут быть перенесены тоже.

Чтобы снять защиту, выполните `werf meta-repo disable --repo registry.mycompany.org/project`. Команда удаляет только маркер; метаданные обратно **не** переносятся, поэтому последующие запуски без `--meta-repo` будут читать устаревшие или пустые метаданные из `--repo`.
