...
1. Сокеты `launchd` недоступны напрямую в контейнерах. Переменная `SSH_AUTH_SOCK`, которая указывает на сокет `launchd` (например `/private/tmp/com.apple.launchd.*`), не будет работать внутри контейнера.

2. Docker Desktop предоставляет свой прокси-сокет. Для доступа к SSH-агенту используется специальный путь `/run/host-services/ssh-auth.sock`, который автоматически пробрасывается в контейнер на macOS.
  
3. Несколько агентов на хосте. Если на macOS запущено несколько SSH-агентов, Docker Desktop выбирает только один (обычно тот, что поднят `launchd` для текущего пользователя). Выбрать другой агент внутри контейнера невозможно.
