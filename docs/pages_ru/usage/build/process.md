...
По умолчанию werf хранит служебные метаданные в основном репозитории (`--repo`) рядом со стадиями образов. При необходимости четыре семейства метаданных — image-metadata, используемые для очистки на основе истории Git, список managed images, метаданные custom-тегов и запись о последней очистке — можно вынести в отдельный репозиторий с помощью `--meta-repo`:
...
Маркер даёт защиту по мере возможностей, а не абсолютную гарантию: он пишется обычной последовательностью «прочитать, затем записать», а container registry не предоставляет атомарного compare-and-set. Два первых запуска, начатых одновременно с разными значениями `--meta-repo`, оба не найдут маркера и оба его запишут. Когда маркер уже существует, проверка выполняется строго.

#### Подключение `--meta-repo` для существующего проекта

Если в `--repo` уже есть метаданные проекта, werf откажется включать `--meta-repo` автоматически (иначе существующие метаданные осиротеют). Сначала перенесите их:

```shell
werf meta-repo migrate --repo registry.mycompany.org/project --meta-repo registry.mycompany.org/project-meta
```

`migrate` копирует четыре семейства метаданных в `--meta-repo` (сначала копирование с проверкой, идемпотентно — можно запускать повторно), записывает маркер в `--repo`, после чего удаляет каждый оригинал из `--repo` — но только после того, как его копия подтверждена в `--meta-repo`. Чтобы сохранить оригиналы, укажите `--remove-source=false`.

Удаление отклоняется целиком, до каких-либо изменений, если среди удаляемых записей есть запись без владеющего проекта: если `--repo` разделяется несколькими проектами ещё с тех времён, когда werf не проставлял метки на эти записи, такая запись может принадлежать другому проекту. Непомеченные записи всё равно копируются, поэтому с `--remove-source=false` они переносятся.
...
В werf v2 нет `--meta-repo`: все четыре семейства метаданных записываются в `--repo`. Чтобы на время перехода обе версии продолжали работать с одним репозиторием, очистка выполняется в два шага: сначала перенос метаданных, накопленных v2, затем очистка.
...
- `--remove-source=false` здесь обязателен. werf v2 продолжает читать все четыре семейства из `--repo`, поэтому поведение `migrate` по умолчанию удалит метаданные, от которых зависит v2.
...
- Не задавайте один и тот же custom-тег (`--add-custom-tag`, `--use-custom-tag`) из обеих версий werf. Сборка v3 записывает алиас в `--repo`, а его метаданные — в `--meta-repo`, тогда как запись, оставленная v2 в `--repo`, при `--remove-source=false` никогда не удаляется. `migrate` считает запись в `--repo` авторитетной и перезаписывает более свежую, после чего очистка может удалить алиас, указывающий на живую стадию.

`werf meta-repo detach` — не обратный перенос: команда удаляет маркер и оставляет метаданные в `--meta-repo`.

`werf purge` маркер тоже не удаляет: команда удаляет стадии и метаданные проекта, но маркер остаётся в `--repo`. Поэтому последующий запуск заново созданного проекта без `--meta-repo` всё равно будет отклонён, пока не выполнить `werf meta-repo detach`.
