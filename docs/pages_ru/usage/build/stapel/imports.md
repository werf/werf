...
- `image: <image name>`: _исходный образ_, имя образа из которого вы хотите копировать файлы или каталоги. Поддерживается как импорт из образов текущего проекта, так и внешние образы в формате `image_name:tag` или `image_name@digest`
