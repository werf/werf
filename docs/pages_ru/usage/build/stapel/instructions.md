...
В приведенном файле конфигурации `werf.yaml` указан _git mapping_, согласно которому содержимое каталона `/src` локального Git-репозитория копируется в каталог `/app` собираемого образа.
