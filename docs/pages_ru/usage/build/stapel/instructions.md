...
Таким образом:
- по умолчанию любое изменение исходных файлов вызывает пересборку и выполнение команд с актуальными исходниками;
- если указать `git.stageDependencies`, пересборка происходит только при изменениях файлов, подпадающих под заданные маски.
