...
Сборка образов через сборщик stapel предполагает описание сборочных инструкций в конфигурационном файле `werf.yaml`. stapel требует BuildKit-бэкенд; бэкенд Docker-сервера сборку stapel-образов не поддерживает.
