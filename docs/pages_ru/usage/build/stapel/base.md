...
Приведенная конфигурация описывает _образ_ `example`, _базовым образом_ для которого является образ с именем `alpine:latest`.

_Базовый образ_ может быть указан с помощью директивы `from`.
...
При использовании внешнего образа указание тега или дайджеста обязательно:
