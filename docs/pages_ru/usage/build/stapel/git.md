...
Опционально может использоваться параметр `basicAuth` для аутентификации по токену в приватных репозиториях.
В качестве источников для токена (параметр `password`) могут использоваться:
* Переменная окружения (`env`)
* Содержимое файла (`src`)
* Произвольное значение (`plainValue`)
