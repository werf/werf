...
> Обратите внимание на ограничение shallow-режима: если тег принудительно перенесен на другой коммит (`git tag -f`), предыдущий коммит не сохраняется в shallow-зеркале, поэтому переиспользование закэшированных стадий, собранных для старой позиции тега, на основе истории коммитов невозможно. Это не затрагивает маппинги с `branch` и первую сборку любого ref.
