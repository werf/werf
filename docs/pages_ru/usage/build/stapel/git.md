...
- `lfs` — загружать объекты [Git LFS](https://git-lfs.com/) и добавлять в образ их содержимое вместо pointer-файлов. По умолчанию выключено. Подробнее рассматривается [в секции Git LFS](#git-lfs);
- `stageDependencies` — список масок файлов и директорий, изменения которых запускают пересборку пользовательских стадий. По умолчанию (если параметр не указан) werf использует маску `**/*`, то есть любое изменение исходников вызывает пересборку стадий с командами сборки. Явные маски позволяют сузить набор файлов, влияющих на пересборку. Более подробно рассматривается [здесь]({{ "usage/build/stapel/instructions.html" | true_relative_url }}).
...
Если указан фиксированный `tag` или `commit`, werf считает такой ref неизменяемым и поддерживает shallow-зеркало удаленного репозитория (`git fetch --depth=1`) вместо получения полной истории. Если нужный коммит уже есть в локальном кэше, werf вообще не обращается к сети. Для `branch`, а также если git-сервер отклоняет shallow-фетч или репозиторий использует сабмодули, werf автоматически переключается на зеркало с полной историей.
...
Файлы, отслеживаемые [Git LFS](https://git-lfs.com/), хранятся в репозитории в виде небольших pointer-файлов, и по умолчанию werf добавляет в образ именно их. Чтобы werf загрузил объекты LFS и добавил в образ их настоящее содержимое, укажите `lfs: true`:
...
Параметр работает как для репозитория проекта, так и для удаленных репозиториев. На сборочном хосте должен быть установлен `git-lfs`: перед архивированием файлов werf выполняет `git lfs pull` для добавляемого коммита, ограничивая загрузку путём `add`. Настройка `lfs.fetchexclude` хоста игнорируется, чтобы результат не зависел от локальной конфигурации git. Если после загрузки какой-либо файл внутри _git mapping_ остаётся pointer-файлом (например, объект отсутствует на LFS-сервере), сборка завершается ошибкой.

Для удаленных репозиториев werf передаёт в `git-lfs` учётные данные из `basicAuth`. Объекты LFS внутри сабмодулей не загружаются.

Поскольку `lfs` меняет добавляемое в образ содержимое, включение или выключение параметра приводит к пересборке _git-стадий_ образа.
...
Опционально может использоваться параметр `basicAuth` для аутентификации по токену в приватных репозиториях. В качестве источников для токена (параметр `password`) могут использоваться:

* Переменная окружения (`env`).
* Содержимое файла (`src`).
* Произвольное значение (`value`).

Пример конфигурации с `basicAuth`:
