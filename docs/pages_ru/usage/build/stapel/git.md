...
- `stageDependencies` — список масок файлов и директорий, изменения которых запускают пересборку пользовательских стадий. Маски задаются для каждой стадии (`install`, `beforeSetup`, `setup`) и выводятся отдельно для каждого _git mapping_: незаданная стадия до последней явно объявленной стадии не получает прямой зависимости от файлов Git-репозитория (`[]`), незаданная стадия после неё получает `**/*`, а если `stageDependencies` не указан вовсе или задан как `{}`, все три стадии получают `**/*`. Более подробно рассматривается [здесь]({{ "usage/build/stapel/instructions.html#маски-незаданных-стадий" | true_relative_url }}).
