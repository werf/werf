...
```
Чтобы использовать секрет в сборке и сделать его доступным для инструкции `RUN`, используйте `--mount=type=secret флаг` в Dockerfile.
При использовании секрета в Dockerfile, секрет монтируется в файл по умолчанию. Путь к файлу секрета по умолчанию внутри контейнера сборки — `/run/secrets/<id>` Обратите внимание, если вы не укажете id секрета в `werf.yaml`, то в качестве id будет исползовано значение по умолчанию: для `env` - имя перменной окружения; для `src` - имя конечного файла (к примеру для `/path/to/file` будет использован `id: file`)
...
Так же вы можете использовать произвольное значение, которое не будет сохранено в конечном образе. Для этого используйте тип `value`.
В этом лучае необходимо явно указать `id` для секрета.
