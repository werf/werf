...
Обратите внимание, если вы не укажете `id` секрета в `werf.yaml`, то в качестве `id` будет использовано значение по умолчанию:
...
Вы также можете использовать произвольное значение, которое не будет сохранено в конечном образе. Для этого используйте тип `value`.  
В этом случае необходимо явно указать `id` для секрета.
