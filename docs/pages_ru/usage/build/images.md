...
> **ЗАМЕЧАНИЕ:** Для того, чтобы использовать секреты в сборках, вы должны явно разрешить их использование в параметрах гитерминизма. Подробнее об этом ([здесь]({{ "/usage/project_configuration/giterminism.html#использование-сборочных-секретов" | true_relative_url }}))
