...
## Ручное управление пересборкой образов

Для ручного управления пересборкой образов можно использовать глобальную директиву `build.cacheVersion`, 
либо её локальную альтернативу `<image>.cacheVersion`. Если указаны обе директивы, локальная имеет приоритет.
