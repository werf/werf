...
- `.helmignore` — список файлов в директории чарта, которые не нужно включать в чарт. Также применяются правила helm по умолчанию, поэтому файлы, начинающиеся с точки, непосредственно в `templates/`, исключаются даже без `.helmignore`.
