...
- `.helmignore` — список файлов в директории чарта, которые не нужно включать в чарт. Также применяются правила helm по умолчанию, поэтому файлы, начинающиеся с точки, непосредственно в `templates/`, исключаются даже без `.helmignore`. Синтаксис правил описан в разделе [Бандлы и чарты]({{ "/usage/distribute/bundles.html#исключение-файлов-или-директорий-из-публикуемого-чарта" | true_relative_url }}).
...
- чарт из OCI/HTTP-репозитория приходит уже упакованным, отфильтрованным своим `.helmignore` на момент упаковки.
