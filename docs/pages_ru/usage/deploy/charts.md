...
- всегда применяется правило Helm по умолчанию `templates/.?*`, поэтому файлы и директории, начинающиеся с точки, непосредственно в `templates/` исключаются даже без `.helmignore`; директория исключается вместе со всем содержимым;
