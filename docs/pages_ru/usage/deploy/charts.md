...
- `.helmignore` — список файлов в директории чарта, которые не нужно включать в чарт, см. ниже.

### Исключение файлов или директорий из чарта

Файл `.helmignore`, находящийся в корне чарта, может содержать фильтры по именам файлов, при соответствии которым файлы или директории не будут добавляться в чарт. Формат правил такой же, как и [в .gitignore](https://git-scm.com/docs/gitignore), за исключением:

- `**` не поддерживается и вызывает ошибку;

- `!` в начале строки работает иначе, чем в `.gitignore`: он исключает всё, что не подошло под маску, — не используйте его;

- `.helmignore` не исключает сам себя по умолчанию;

- всегда применяется правило Helm по умолчанию `templates/.?*`, поэтому файлы и директории, начинающиеся с точки, непосредственно в `templates/` исключаются даже без `.helmignore`, директория — вместе со всем содержимым;

- правило с косой чертой на конце соответствует только директориям, поэтому не исключает символическую ссылку, которую werf не может разрешить, — пишите такое правило без косой черты;

- файлы, доставленные в чарт через includes, фильтруются теми же правилами. `.helmignore`, пришедший из include, действует только когда у чарта нет своего, и тогда фильтрует и локальные файлы чарта.

Какой `.helmignore` фильтрует зависимый чарт, зависит от способа его подключения:

- чарт в директории `charts` загружается как часть родительского чарта, поэтому фильтруется файлом `.helmignore` родительского чарта, а его собственный `.helmignore` не действует;

- чарт, подключённый через `dependencies[].repository: file://`, загружается как отдельный чарт, поэтому фильтруется своим собственным `.helmignore`, а правила родительского чарта к нему не применяются;

- чарт из OCI/HTTP-репозитория приходит упакованным: его `.helmignore` применён при упаковке.
