...
- `.helmignore` — список файлов в директории чарта, которые не нужно включать в чарт. Правила применяются при любом чтении чарта, в том числе в `werf render`, `werf converge`, `werf plan` и `werf lint`. Также применяются собственные правила helm по умолчанию, поэтому файлы, начинающиеся с точки, непосредственно в `templates/` (например, `templates/.gitkeep`) исключаются даже если у чарта нет `.helmignore`.
