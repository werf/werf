...
Помимо [Helm-шаблонов]({{ "/usage/deploy/templates.html" | true_relative_url }}), werf поддерживает генерацию Kubernetes-манифестов с помощью TypeScript. Helm-шаблоны и TypeScript-шаблоны могут сосуществовать в одном чарте — полученные манифесты объединяются в единый мульти-документный YAML.

TypeScript-шаблоны работают из коробки: развёртывание чарта с директорией `ts/` не требует дополнительных инструментов или настройки — рантайм Deno TypeScript поставляется внутри самого werf и рендерит TypeScript-шаблоны.
...
- Изолированные окружения — модули npm по умолчанию включаются в бандл чарта, а рантайм Deno поставляется внутри werf, так что во время развёртывания не будет сетевых обращений, кроме обращений к самому Kubernetes.
...
Просто запустите `werf converge`: бинарный файл Deno будет распакован из werf в кеш, TypeScript-шаблоны будут отрендерены и развёрнуты.
...
Рантайм Deno поставляется внутри werf, поэтому в изолированном окружении достаточно, чтобы сам чарт был самодостаточным:
...
2. На целевой машине в изолированном окружении (без доступа к сети) выполните:
   ```shell
   werf bundle apply --repo example.org/mycompany/myapp
   ```
   TypeScript-шаблоны будут отрендерены и развёрнуты с использованием предварительно скомпилированных файлов из бандла чарта.
