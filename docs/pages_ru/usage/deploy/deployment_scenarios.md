...
Во многих CI/CD-системах сборка и развертывание выполняются в разных заданиях — зачастую на разных раннерах, с разными правами доступа или даже в разное время (например, ручное подтверждение перед деплоем в production). [Отчёт по сборке]({{ "/usage/build/process.html#отчёт-по-сборке" | true_relative_url }}) выступает контрактом между этими этапами: задание сборки сохраняет отчёт с именами образов, тегами и дайджестами, а задание развертывания читает его через `--use-build-report`, полностью пропуская сборку.

Типичные причины разделения сборки и развертывания:
- **Разные раннеры**: сборка на мощной машине с Docker, деплой с легковесного раннера с доступом к кластеру.
- **Разные права доступа**: задание сборки пушит в registry, задание деплоя применяет в Kubernetes — принцип минимальных привилегий.
- **Ручные гейты**: сборка при каждом пуше, деплой в production — только после подтверждения.
- **Повторное использование**: собрать один раз, развернуть на staging и production из одного отчёта.
...
Формат envfile нативно интегрируется с [dotenv-артефактами GitLab CI](https://docs.gitlab.com/ci/yaml/artifacts_reports/#artifactsreportsdotenv). Задание сборки сохраняет отчёт как dotenv-артефакт, а задание развертывания автоматически получает переменные с именами образов:
...
Флаг `--use-build-report` также поддерживают команды `werf export`, `werf render`, `werf plan`, `werf run`, `werf kube-run`, `werf lint`, `werf compose`, `werf bundle publish`, `werf helm get-autogenerated-values` и `werf stages copy`.
