...
Аннотация `werf.io/resource-policy` определяет, какие операции werf может выполнять с ресурсом — создание, обновление, пересоздание и удаление, — и вдохновлена аннотацией `helm.sh/resource-policy`. Допустимые значения:
* `skip-create`: ресурс не создаётся, если он отсутствует в кластере
* `skip-update`: ресурс не обновляется, если он уже есть в кластере
* `skip-recreate`: ресурс не пересоздаётся
* `skip-delete`: ресурс не удаляется при удалении из чарта или при удалении релиза
* `keep`: Helm-совместимый псевдоним для `skip-delete`

Можно указать несколько значений одновременно. Пересоздания, вызванные аннотациями `werf.io/delete-policy` или `helm.sh/hook-delete-policy`, пропускаются значением `skip-recreate`, а вызванные ими удаления — значением `skip-delete`.

Значения `skip-create`, `skip-update` и `skip-recreate` работают, только если аннотация задана в чарте, а `skip-delete` работает ещё и в том случае, если аннотация задана на ресурсе в кластере.

По умолчанию для обычных ресурсов политика ресурса не задана. Для Namespace релиза `skip-delete` устанавливается всегда — в дополнение к политике, заданной на нём.
...
Аннотация `werf.io/resource-policy` даёт ту же защиту и больше вариантов, но полностью заменяет эту аннотацию: если заданы обе, `helm.sh/resource-policy` игнорируется, поэтому для сохранения защиты от удаления в `werf.io/resource-policy` нужно указать `skip-delete` (или `keep`).
