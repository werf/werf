...
* `skip-create`: не создавать ресурс, если он отсутствует в кластере.
* `skip-update`: не обновлять ресурс, если он уже есть в кластере.
* `skip-recreate`: не пересоздавать ресурс.
* `skip-delete`: не удалять ресурс при удалении из чарта или при удалении релиза.
