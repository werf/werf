...
Аннотация `werf.io/resource-policy` защищает ресурс от создания, изменения или удаления и вдохновлена аннотацией `helm.sh/resource-policy`. Допустимые значения:
...
Можно указать несколько значений одновременно. Пересоздания и удаления, вызванные аннотациями `werf.io/delete-policy` или `helm.sh/hook-delete-policy`, также пропускаются значениями `skip-recreate` и `skip-delete`.
...
По умолчанию у обычных ресурсов политика не задана, а у Namespace релиза используется `skip-delete`.
...
Аннотация `werf.io/resource-policy` даёт такую же защиту с бо́льшим количеством вариантов и имеет приоритет над этой аннотацией.
