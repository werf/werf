...
`werf cleanup` предусматривает опцию `--whitelist`. Это опция позволяет передать _белый список_ тегов стадий, чтобы защитить их от удаления.

Помогает сформировать _белый список_ команда `werf cleanup --dry-run`. Она выводит лог с учетом цветов:
