...
`werf cleanup` предусматривает опцию `--whitelist`. Это опция позволяет передать _белый список_ тегов (стадий/образов), чтобы защитить их от удаления.

Помогает сформировать _белый список_ команда `werf cleanup --dry-run`. Она выводит лог с учетом цветов:

 + <span style="color: green;">Зеленый цвет</span> -- образы, которые будут сохранены.
 + <span style="color: red;">Красный цвет</span> -- образы, которые будут удалены.
