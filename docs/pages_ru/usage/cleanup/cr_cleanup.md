...
Когда используется несколько изолированных окружений развёртывания, следовательно:

+ Нет возможности почистить общий container registry с учётом используемых образов **во всех** окружениях.
+ Нет возможности почистить каждый container registry (есть общий и отдельный под каждое окружение) с учётом особенностей каждого окружения.

`werf cleanup` предусматривает опцию `--keep-list`. Это опция позволяет передать _список сохранения_ тегов стадий, чтобы защитить их от удаления.

Помогает сформировать _список сохранения_ команда `werf cleanup --dry-run`. Она выводит лог с учетом цветов:
