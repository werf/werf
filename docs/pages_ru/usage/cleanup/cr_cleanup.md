...
Элемент `imageMetadata` указывает имя образа в поле `imageName`, за исключением случая, когда метаданные относятся к образу, которого больше нет в `werf.yaml` — восстановить имя в этом случае невозможно, поэтому элемент содержит внутренний идентификатор метаданных в поле `id` и не содержит `imageName`. Команда `werf purge` удаляет все метаданные, не обращаясь к `werf.yaml`, поэтому в её отчёте всегда используется `id`.

`repo` — это хранилище стадий, против которого выполнялась очистка, а `finalRepo` появляется, если настроен финальный репозиторий.

Адрес не дублируется в каждом элементе — он определяется по полю `type`:
...
Правило нормативное: если у элемента нет собственного адреса, он удалён из `repo`.
...
Те же опции поддерживаются командой `werf purge`, а также `werf host purge` при вызове с `--project-name`. Обе удаляют всё, поэтому в их отчётах массив `kept` всегда пуст, а `command` равен `purge` и `host purge` соответственно. Команда `werf host cleanup` отчёт не пишет: она очищает локальное хранилище хоста и container registry не затрагивает.
