...
Опция `--keep-list=/path/to/list.txt` позволяет передать список тегов в виде файла. `list.txt` -- это файл, 
в котором каждый тег указан на отдельной строке. Например:
...
Сформировать данный список может помочь раздел документации по [сохранению результата работы](#сохранение-результата-работы).
...
В ходе работы `werf cleanup` выводит теги стадий с учетом цветов:

+ <span style="color: green;">Зеленый цвет</span> -- тег стадии, который нужно сохранить.
+ <span style="color: red;">Красный цвет</span> -- тег стадии, который нужно удалить.

**Пример**. Используя опцию `--dry-run` и знания о цвете, выведем список тегов, 
которые помечены _к удалению_ (<span style="color: red;">красным</span>):
