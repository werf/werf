...
Для выката приложения в Kubernetes werf предполагает использование режима сборки с публикацией образов в репозитории Container Registry. Такой режим включается при использовании команды `werf build --repo REPO`. В разделе [организация хранения образов в Container Registry]({{ "usage/build_draft/storage.html" | true_relative_url }}) рассмотрены различные схемы хранения и дополнительные связанные опции, которые предоставляет werf.
