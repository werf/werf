...
## Выбор хранилища сборки

<!-- прим. для перевода: на основе https://werf.io/documentation/v1.2/internals/stages_and_storage.html#storage -->

Все слои собираемых образов сохраняются в так называемое **хранилище** по мере сборки. Есть два вида хранилища: локальное и репозиторий (Container Registry).
...
<div class="details">
<a href="javascript:void(0)" class="details__summary">Как работает хранилище в репозитории</a>
<div class="details__content" markdown="1">

werf по умолчанию реализует послойное кеширование в Container Registry при использовании параметра `--repo REPO`.
...
2. Выбираются все стадии, подходящие под дайджест, т.к. с одним дайджестом может быть связанно несколько стадий в хранилище.
...
- `TIMESTAMP_MILLISEC` — временная отметка, которая проставляется в процессе [процедуры сохранения слоя в Container Registry](#выбор-хранилища-сборки) после того как стадия была собрана.
...
Сервис синхронизации — это компонент werf, который предназначен для координации нескольких процессов werf и выполняет роль _менеджера блокировок_. Блокировки требуются для корректной публикации новых образов в Container Registry и реализации алгоритма сборки, описанного в разделе [как работает хранилище в репозитории](#выбор-хранилища-сборки).
...
Множество процессов werf, работающих с одним и тем же проектом должны использовать одинаковое хранилище и адрес сервера синхронизации. В случае если использование публичного сервиса `https://synchronization.werf.io` не подходит, есть 3 варианта:
...
> **ЗАМЕЧАНИЕ:** Сборочный бекэнд Buildah на данный момент не поддерживает [хранение образов в локальном хранилище](#выбор-хранилища-сборки), поддерживается только [хранение образов в Container Registry](#выбор-хранилища-сборки).
