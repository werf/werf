...
Файл `.helmignore`, находящийся в корне чарта, может содержать фильтры по именам файлов, при соответствии которым файлы или директории не будут добавляться в чарт. Правила применяются при любом чтении чарта, а не только при публикации. Формат правил такой же, как и [в .gitignore](https://git-scm.com/docs/gitignore), за исключением:

- `**` не поддерживается: правило с ним прерывает команду ошибкой `double-star (**) syntax is not supported`;

- `!` в начале строки не поддерживается;

- `.helmignore` не исключает сам себя по умолчанию;

- всегда применяется правило helm по умолчанию `templates/.?*`, поэтому файлы, начинающиеся с точки, непосредственно в `templates/` (например, `templates/.gitkeep`) исключаются даже если у чарта нет `.helmignore`.
