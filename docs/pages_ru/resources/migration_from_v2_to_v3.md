...
**Старые базовые образы с `WERF_COMMIT_*`.** Старые сборки werf могли сохранять `WERF_COMMIT_HASH`, `WERF_COMMIT_TIME_HUMAN` и `WERF_COMMIT_TIME_UNIX` из сборочного контейнера в конфигурации образа. Утечка исправлена: теперь werf передаёт эти переменные только на время выполнения команд сборки. Однако переменные можно унаследовать от старого базового образа. imageSpec больше не удаляет их автоматически при модификации env; если они есть в вашем базовом образе, добавьте их в `imageSpec.config.removeEnv`.
...
Значение по умолчанию для `git.stageDependencies` изменилось **для каждой из стадий `install`, `beforeSetup`, `setup`**:

| Настройка | Было — v2 | Стало — v3 |
|---|---|---|
| Маски не заданы или список пуст | Изменения файлов Git не инвалидировали стадию через `stageDependencies`. | Эквивалент `**/*`: учитываются все файлы git mapping с его фильтрами `includePaths`/`excludePaths`. |
| Маски заданы явно | На контрольную сумму стадии влияли только подходящие файлы. | То же поведение: масками можно сузить зависимости дорогих стадий. |

**По умолчанию безопаснее:** изменения исходников запускают команды сборки заново, не требуя ручной настройки зависимостей. Сборки могут стать чаще. Если задаёте маски, включите все файлы, влияющие на результат стадии; пустой список больше не отключает отслеживание файлов.

Непустой `stageDependencies` для стадии без инструкций сборки теперь вызывает **ошибку вместо предупреждения**. Исправьте имя стадии, добавьте отсутствующие инструкции или удалите неиспользуемую настройку. Подробнее — [зависимость от изменений в Git-репозитории]({{ "/usage/build/stapel/instructions.html#зависимость-от-изменений-в-git-репозитории" | true_relative_url }}).
...
**Обновление файлов из Git в legacy Stapel.** При стандартных `git.stageDependencies` изменения исходников запускают команды сборки заново: дополнительных действий из-за этого изменения не требуется. Обновление затрагивает файлы внутри образа, а не рабочую копию Git.

Проверка нужна конфигурациям, где **сужены `git.stageDependencies` и команды изменяют исходные файлы**. Если файл изменился в Git, но стадия обработки взята из кеша, файл заменяется версией из Git, теряя изменения команды. Раньше конфликт мог остановить сборку; теперь она может успешно завершиться с неожиданным содержимым образа.

- Включите файлы, требующие повторной обработки, в `git.stageDependencies` соответствующей стадии либо сохраняйте результат обработки отдельно от исходников.
- Проверьте повторную сборку после изменения такого файла, а не только чистую сборку.

Текстовые и бинарные файлы теперь обновляются одним способом — копированием нужных версий из Git. Это устраняет ошибки применения текстовых патчей, но больше не выявляет конфликты с изменениями команд сборки.

### Удалённые параметры сборки
...
| `WERF_STAGED_DOCKERFILE_VERSION=v1` | Убрать настройку: staged Dockerfile всегда использует код версии v2. |
