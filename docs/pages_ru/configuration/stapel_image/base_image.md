...
> Примеры проблем, которые может вызвать это поведение в CI процессах (например, в pipeline GitLab):
  ...
  <br />
  <br />
  Если вы хотите использовать функционал данной директивы, добавьте также параметр _herebyIAdmitThatFromLatestMightBreakReproducibility: true_ рядом с параметром _fromLatest_.
