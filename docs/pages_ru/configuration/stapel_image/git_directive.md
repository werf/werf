...
> Примеры проблем, которые может вызвать это поведение в CI процессах (например, в pipeline GitLab):
  ...
  <br />
  <br />
  Если вы хотите использовать функционал данной директивы, добавьте также параметр __herebyIAdmitThatBranchMightBreakReproducibility: true_ в секцию конфигурации удаленного git-репозитория.
