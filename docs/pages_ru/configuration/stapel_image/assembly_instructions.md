...
werf при сборке использует собственный исполняемый файл bash и вам не нужно отдельно добавлять его в образ (или [базовый образ]({{ site.baseurl }}/configuration/stapel_image/base_image.html)) при сборке.
...
Предпочтительный способ копирования файлов в образ — использование [_git mapping_]({{ site.baseurl }}/configuration/stapel_image/git_directive.html).
...
- в файлах, импортируемых из [артефактов]({{ site.baseurl }}/configuration/stapel_artifact.html)
...
Как описывалось в статье про [_git mapping_]({{ site.baseurl }}/configuration/stapel_image/git_directive.html), существуют специальные стадии _gitArchive_ и _gitLatestPatch_.
...
Параметры _CacheVersion_ можно использовать совместно с [шаблонами Go]({{ site.baseurl }}/configuration/introduction.html#go-templates), чтобы определить зависимость _пользовательской стадии_ от файлов, не находящихся в git-репозитории.
