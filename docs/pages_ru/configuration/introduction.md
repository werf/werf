...
Директивы `deploy` и `cleanup` вынесены в отдельные статьи, [развертывание в Kubernetes]({{ site.baseurl }}/documentation/advanced/helm/basics.html) и [политики очистки]({{ site.baseurl }}/documentation/configuration/cleanup.html). 

#### Имя проекта

Ключ `project` определяет уникальное имя проекта вашего приложения. 
Имя проекта влияет на имена образов в _stages storage_, namespace в Kubernetes, имя Helm-релиза и зависящие от него имена (смотри подробнее про [развертывание в Kubernetes]({{ site.baseurl }}/documentation/advanced/helm/basics.html)). 
...
Образ артефакта — это вспомогательный образ, предназначенный для изолирования сборочного окружения, инструментов и данных, от конечного образа приложения (подробнее [здесь]({{ site.baseurl }}/documentation/internals/building_images_with_stapel/artifact.html)).
