...
Для сборки образа backend будем использовать два артефакта (подробней об артефактах можно прочитать [здесь]({{ site.baseurl }}/documentation/internals/building_images_with_stapel/artifact.html)) — `storefront` и `appserver`.
