...
Для сборки образа backend будем использовать два артефакта (подробней об артефактах можно прочитать [здесь]({{ site.baseurl }}/configuration/stapel_artifact.html)) — `storefront` и `appserver`.
...
Папка `/usr/share/maven/ref/repository` монтируется с помощью инструкции `build_dir`, чтобы заработало кэширование (подробнее об инструкциях монтирования можно прочитать [здесь]({{ site.baseurl }}/configuration/stapel_image/mount_directive.html)).
...
* Обеспечение совместного использование данных с помощью монтирования (подробнее об этом можно прочитать [здесь]({{ site.baseurl }}/configuration/stapel_image/mount_directive.html)).
* Использование общих служебных образов (подробнее [здесь]({{ site.baseurl }}/configuration/stapel_image/import_directive.html)).
