...
В строке `image: node` дано название для образа, который соберёт werf. Это имя мы впоследствии будем указывать при запуске контейнера. Строка `from: node:14-stretch` определяет, что будет взято за основу, мы берем официальный публичный образ с нужной нам версией ruby.
...
TODO: надо разобраться с этим, нахер они тут
```yaml
  stageDependencies:
    install:
    - package.json
...
TODO: пределать это нахуй. У автора предлагалось писать логи в console.log и console.error. Не уверен, что это корректно.
