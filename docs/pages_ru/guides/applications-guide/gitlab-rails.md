...
Мы реализуем два стенда: production и staging. 
...
Подробную информацию о конфигурации ci-env можно найти [тут](https://werf.io/documentation/reference/plugging_into_cicd/overview.html). 
От себя лишь хочется добавить, что если вы используете совместно с Gitlab внешний registry (harbor,Docker Registry,Quay etc.), то необходимо глобально определить адрес репозитория для образов (`WERF_IMAGES_REPO`) и stages (`WERF_STAGES_STORAGE`) (в переменных окружения проекта или явно в gitlab-ci.yaml), как это сделать можно узнать [тут](https://werf.io/documentation/cli/main/build_and_publish.html). И так же не забыть первой командой выполнить [docker login](https://docs.docker.com/engine/reference/commandline/login/).
...
Плюс стадий в том, что они имеют собственный тэг, который представляет собой хэш содержимого нашего образа. Тем самым позволяя полностью избегать не нужных пересборок наших образов. Стадии переиспользуются между сборками, тем самым резко снижается время доставки кода. Стадии полностью изолированы: стадия используются только лишь в том случае, если коммит используемый при сборке этой стадии является родительским по отношению к собираемому коммиту. Таким образом, стадии двух веток, которые ответвились от master, никогда не пересекаются и сборки не блокируют друг друга.
...
Сборочные инструкции описываются в пользовательских стадиях. Разбиение команд на эти стадии, а также правильные зависимости от изменений в исходниках — ключ к эффективной инкрементальной сборке:

*   стадия beforeInstall для инсталляции системных пакетов;
*   стадия install для инсталляции системных зависимостей и зависимостей приложения;
*   стадия beforeSetup для настройки системных параметров и установки приложения;
*   стадия setup для настройки приложения.
