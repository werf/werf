...
TODO: кратко рассказать, как отлаживать разработку артефактов: что можно просто переименовать дирекиву artefact в image и получить образ, который можно уже отладить.
{% endofftopic %}
...
Мы запустим оба контейнера одним Deployment-ом и все запросы будет приходить сперва на nginx. Тот будет перенаправлять запрос приложению если не найдётся статических файлов.
