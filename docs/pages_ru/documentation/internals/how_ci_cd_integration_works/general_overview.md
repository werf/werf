...
Согласно процедуре [интеграции с container registry](#интеграция-с-container-registry), команда [`werf ci-env`]({{ "documentation/reference/cli/werf_ci_env.html" | true_relative_url }}) определяет container registry и устанавливает параметр `--repo`, используя переменную окружения `WERF_REPO`.

#### WERF_REPO_CONTAINER_REGISTRY

Согласно процедуре [интеграции с container registry](#интеграция-с-container-registry), команда [`werf ci-env`]({{ "documentation/reference/cli/werf_ci_env.html" | true_relative_url }}) устанавливает параметр `--repo-container-registry`, используя переменную окружения `WERF_REPO_CONTAINER_REGISTRY`, вдобавок к [`WERF_REPO`](#werf_repo).
