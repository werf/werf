...
Пользовательская стадия — это стадия, инструкции для сборки которой задаются пользователем werf.

Инструкции задаются через config или chef-рецепты — зависит от используемого сборщика: shell сборщик или [chef сборщик]({{ site.baseurl }}/ruby/chef.html).
...
Данная стадия производит скачивание указанного базового образа (фактически docker pull) и фиксирует его в кэше werf.
...
К примеру, git artifact стадии считаются пустыми, если при описании приложения в config не были использованы git-artifact-ы (git directive), аналогичная ситуация с artifact-ами и пользовательскими стадиями.
