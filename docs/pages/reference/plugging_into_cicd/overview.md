...
As part of the [Docker registry integration](#docker-registry-integration) procedure, [`werf ci-env` command]({{ site.baseurl }}/documentation/cli/toolbox/ci_env.html) identifies the Docker registry and defines the `--images-repo` parameter based on the `WERF_IMAGES_REPO` environment variable.
