...
As part of the [Docker registry integration](#docker-registry-integration) procedure, [`werf ci-env` command]({{ site.baseurl }}/documentation/cli/toolbox/ci_env.html) identifies the Docker registry and defines the `--repo` parameter using `WERF_REPO` environment variable.
