...
- [converge]({{ "documentation/reference/cli/werf_converge.html" | true_relative_url }}) — to install or update an application;
...
Secret files are placed in the `.helm/secret` directory. The user can create an arbitrary files structure in this directory. [This article about secrets]({{ "documentation/advanced/helm/working_with_secrets.html#secret-file-encryption" | true_relative_url }}) describes how to encrypt them.
...
Values placed under the `global` key will be available both in the current chart and in all [subcharts]({{ "documentation/advanced/helm/working_with_chart_dependencies.html" | true_relative_url }}).

Values placed under the arbitrary `SOMEKEY` key will be available in the current chart and in the `SOMEKEY` [subchart]({{ "documentation/advanced/helm/working_with_chart_dependencies.html" | true_relative_url }}).
...
The [werf helm list]({{ "documentation/reference/cli/werf_helm_list.html" | true_relative_url }}) command lists releases created by werf. Also, the user can fetch the history of a specific release with the [werf helm history]({{ "documentation/reference/cli/werf_helm_history.html" | true_relative_url }}) command.
...
Tracking can be configured for each resource using [resource annotations]({{ "documentation/reference/deploy_annotations.html" | true_relative_url }}), which should be set in the chart templates.
