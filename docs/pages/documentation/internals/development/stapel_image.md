...
werf mounts _stapel image_ into each build container when building docker images with _stapel builder_ to enable ansible, git service operations and for other service purposes. More info about _stapel builder_ are available [in the article]({{ "documentation/internals/building_of_images/build_process.html#stapel-image-and-artifact" | relative_url }}).
