...
* Могут использоваться такие же директивы, как у dimg, но с некоторыми особенностями:
  * Не используется директива [docker](docker_directive.html).
...
Dapp поддерживает 2 [типа сборщика](#тип-сборщика): shell сборщик] и [ansible сборщик](https://github.com/flant/dapp/blob/master/docs/rfc/yaml_with_ansible.md).

* В одном [dimg](#dimg) может быть использован один тип сборщика.
  * Соответственно существует 2 типа dimg: shell dimg и [ansible dimg](https://github.com/flant/dapp/blob/master/docs/rfc/yaml_with_ansible.md).
...
* Используется для хранения:
  * [ansible playbook'ов](https://github.com/flant/dapp/blob/master/docs/rfc/yaml_with_ansible.md).
  * Артефактов.
