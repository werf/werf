...
Для добавления кода в собираемый образ, предусмотрена директива `git`, с помощью которой можно из .git репозитория и его submodul'ей добавить код как в образ приложения так и в образ артефакта.

При первой сборке образа с указанием директивы `git`, в него добавляется указанное в директиве содержимое git репозитория (git archive). При последующих сборках добавляются только docker-слои содержащие изменения, т.е. Git-патчи (git patch apply). Содержимое таких docker-слоев с патчами также кэшируется, что еще более повышает скорость сборки. Причем, в случае отмены сделанных изменений в исходном коде приложения (например, через git revert), при сборке будет накладываться патч с отменой изменений, будет использоваться слой из кеша.

Система кеширования dapp принимает решение о необходимости повторной сборки стадии или использовании кеша, на основании вычисления сигнатуры стадии - т.е. на основании изменения инструкций стадии в dappfile. Таким образом, изменение кода в репозитории не приведет к повторной сборке какой-либо стадии. В директиве `git` предусмотрена возможность явного указания зависимостей от файлов и папок, при изменении которых необходимо выполнить принудительную сборку определенных стадий, не используя кеш. Эта возможность реализуется указанием названии стадии в директиве `stageDependencies` (`stage_dependencies` для Ruby синтаксиса).

Правильная установка зависимостей - важное условие построения эффективного процесса сборки. Для исключения избыточного копирования кода и более точного описания зависимостей, в параметре `stageDependencies` можно  сузить зависимость с помощью директив `includePaths` и `excludePaths`.

Важно понимать, что добавление в образ описанного в директиве `git` кода, происходит на стадии `install`, т.е. на стадии `before install` код еще не доступен (см. [подробней](/architect/stages_arhitecture.html) про стадии сборки).

Количество указаний директивы `git` в описании образа не ограничено, но нужно стремиться к их уменьшению, путем правильного использования `includePaths` и `exckudePaths`.
...
* изменение кода в git репозитории накладывается патчем и не ведет к пересборке какой-либо стадии
