...
|                                                       | Очистка данных **одного проекта**                                      | Очистка данных **всех проектов** |
|-------------------------------------------------------|------------------------------------------------------------------------|----------------------------------|
| Очистка неактуальных данных в **container registry**  | `werf cleanup --repo REPO`                                             | не поддерживается                |
| Полная очистка **container registry**                 | `werf purge --repo REPO` (частичная поддержка)                         | не поддерживается                |
| Очистка неактуальных данных **хоста**                 | `werf host cleanup --project-name PROJECT` (частичная поддержка)       | `werf host cleanup`              |
| Полная очистка **хоста**                              | `werf host purge --project-name PROJECT` (частичная поддержка)         | `werf host purge`                |

Следует отметить, что:
 - **Очистку неактуальных данных** данных безопасно вызывать **в любой момент времени**, вручную или **автоматически**, с малыми рисками потерять критически важные данные, которые используются в production.
