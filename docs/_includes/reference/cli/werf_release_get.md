...
```shell
werf release get [revision] [options]
```
...
```shell
      ...
      --output-format="yaml"
            Output format. Options: yaml, json (default $WERF_OUTPUT_FORMAT or "yaml")
      --print-values=false
            Include computed values in output (default $WERF_PRINT_VALUES or false)
      --release=""
