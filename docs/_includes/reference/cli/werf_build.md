...
```shell
      ...
      --build-report-operations=false
            Collect low-level operations statistics for the whole command run: add Operations and   
            StageCache sections to the build report and print operations summary before the command 
            exits (by default $WERF_BUILD_REPORT_OPERATIONS or false). Also enabled by --log-debug
      --build-report-path=""
