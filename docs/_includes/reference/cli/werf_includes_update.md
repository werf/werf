...
{% assign header = "###" %}
{% endif %}
Create or update includes lock file by resolving git references in the includes config to their latest commits (default: werf-includes.lock).

{{ header }} Syntax

```shell
werf includes update [flags] [options]
```
