...
The following commands support `--save-build-report`: `werf build`, `werf converge`, `werf plan`, `werf render`, `werf lint`, `werf bundle publish`, `werf helm get-autogenerated-values`, `werf stages copy`.
...
Where `<IMAGE>` is the uppercased image name with `/`, `-`, `.` replaced by `_`. For unnamed images, the prefix is `WERF_`.
...
The following commands support `--use-build-report`: `werf converge`, `werf plan`, `werf render`, `werf export`, `werf run`, `werf kube-run`, `werf lint`, `werf compose`, `werf bundle publish`, `werf helm get-autogenerated-values`, `werf stages copy`.
