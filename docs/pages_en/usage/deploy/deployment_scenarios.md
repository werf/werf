...
In many CI/CD setups, the build and deployment stages run in different jobs — often on different runners, with different permissions, or even at different times (e.g. manual approval before production deploy). A [build report]({{ "/usage/build/process.html#build-report" | true_relative_url }}) acts as a contract between these stages: the build job saves the report with image names, tags, and digests, and the deployment job reads it via `--use-build-report`, skipping the build entirely.

Typical reasons to split build and deploy:
- **Different runners**: build on a powerful machine with Docker, deploy from a lightweight runner with cluster access.
- **Different permissions**: the build job pushes to the registry, the deploy job applies to Kubernetes — principle of least privilege.
- **Manual gates**: build on every push, deploy to production only after approval.
- **Reuse builds**: build once, deploy to staging and production from the same report.
...
The envfile format integrates natively with [GitLab CI dotenv artifacts](https://docs.gitlab.com/ci/yaml/artifacts_reports/#artifactsreportsdotenv). The build job saves the report as a dotenv artifact, and the deploy job receives the image variables automatically:
...
The `--use-build-report` flag is also supported by `werf export`, `werf render`, `werf plan`, `werf run`, `werf kube-run`, `werf lint`, `werf compose`, `werf bundle publish`, `werf helm get-autogenerated-values`, and `werf stages copy`.
