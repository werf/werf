...
werf supports passing secrets as:
 - separate [secret values]({{ "/usage/deploy/configuration/values.html#user-defined-secret-values" | true_relative_url }}) yaml file (`.helm/secret-values.yaml` by default, or any file passed by the `--secret-values` option);
