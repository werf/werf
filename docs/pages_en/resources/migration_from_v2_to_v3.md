...
1. Newly encrypted secrets use authenticated AES-GCM: whole secret files use format 2 and scalar values in `secret-values.yaml` use format 3. Existing format-16 AES-CBC values remain readable, but a new format-2/3 value is unreadable by werf before v3 and nelm before v2. Upgrade every CI job and saved deploy-plan reader before creating, editing, or rotating secrets; no downgrade writer exists. Existing encrypted scalars stay strings because format 16 did not store their type, so edit each value to restore its intended type. Comments on encrypted values are now preserved in cleartext; never put secrets in them. The `key: <whole-blob ciphertext>` form in secret-values remains supported.
