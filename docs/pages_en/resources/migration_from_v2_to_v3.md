...
1. Image names are validated against `^[A-Za-z0-9][A-Za-z0-9_.-]*(/[A-Za-z0-9][A-Za-z0-9_.-]*)*$`. Letters, digits, `_`, `.`, `-` and slash-separated segments stay valid, so hierarchical names such as `modules/controller` keep working; a name that is empty, contains whitespace, or has an empty segment (`/api`, `modules//controller` — typically a Go template that rendered to nothing) is now a config error instead of an obscure failure later in the build.
