...
1. `.helmignore` is now applied when werf reads the chart: the files it lists disappear from the rendered manifests and from the published bundle. Helm's default rules apply even in a chart that has no `.helmignore` and exclude dotfiles directly under `templates/`. A rule containing `**` or starting with `!` used to have no effect: `**` now raises an error, and `!` is best avoided — unlike in `.gitignore` it does not make an exception to the previous rules but excludes everything that does not match the pattern, leaving the chart empty. Which `.helmignore` filters a dependent chart depends on how the chart is included, see [Charts and dependencies]({{ "/usage/deploy/charts.html" | true_relative_url }}).
