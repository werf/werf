...
1. Deployment commands no longer use `HELM_DRIVER` to select release storage. Replace it with `WERF_RELEASE_STORAGE` before upgrading: otherwise werf uses its default release storage instead of the one selected by `HELM_DRIVER`. This does not remove Helm's path variables such as `HELM_CACHE_HOME`, `HELM_CONFIG_HOME`, and `HELM_DATA_HOME`.
...
1. Image names are validated at config load. A name consists of latin letters, digits, `_`, `.`, `-` and `+`, optionally split into slash-separated segments, and every segment must start with a letter or a digit and end with a letter, a digit or `+`. Hierarchical names such as `modules/controller` keep working; a name that is empty, contains whitespace, or has a dangling separator (`/api`, `api-`, `modules//controller` — typically a Go template that rendered to nothing) is now a config error instead of an obscure failure later in the build.
...
1. `network:` in `werf.yaml` and `--backend-network` are now honored for non-staged Dockerfile builds on the native Buildah backend, where they used to be silently ignored. Images built with `staged: true` still ignore the image-level setting; only a per-instruction `RUN --network=` flag applies there. That backend accepts only `default`, `host` and `none`, so a Docker-style value such as `bridge` or a named network is now rejected with an error instead of being dropped. `none` still cannot be honored under `native-chroot`, because Buildah forces host networking for chroot isolation.
1. The native Buildah backend now uses `netavark` as its network backend and `pasta` as its rootless network command instead of CNI and slirp4netns. The `netavark` binary must be present in a Podman helper binaries directory (`/usr/local/libexec/podman`, `/usr/local/lib/podman`, `/usr/libexec/podman` or `/usr/lib/podman`; `$PATH` is not searched) on every host that runs werf in Buildah mode — without it werf fails at backend initialization, even in chroot mode with no network-using instructions. A rootless build that configures a network additionally requires the `pasta` binary from the `passt` package (found in a helper binaries directory or via `$PATH`); `network: host`, `network: none` and chroot mode never invoke it, because Buildah only configures a network for a non-host namespace. CNI support is compiled out and cannot be restored; slirp4netns is no longer used, but can be brought back per host with `CONTAINERS_CONF_OVERRIDE` (a config with `default_rootless_network_cmd="slirp4netns"` in the `[network]` section). Pinning the network backend bypasses `${graphroot}/defaultNetworkBackend`: werf neither reads nor rewrites that file, so a storage whose sentinel says `cni` keeps saying it and still steers a system podman or buildah CLI sharing the same graphroot. The official Ubuntu werf images are now based on Ubuntu 24.04.
1. `.helmignore` is now applied when werf reads the chart: the files it lists disappear from the rendered manifests and from the published bundle, without any warning, so check `werf plan` before the first `werf converge`. Helm's default rules apply even in a chart that has no `.helmignore` and exclude files and directories starting with a dot directly under `templates/`, a directory along with its contents. A rule containing `**` or starting with `!` used to have no effect: `**` now raises an error, and `!` is best avoided — unlike in `.gitignore` it does not make an exception to the previous rules but excludes everything that does not match the pattern, usually leaving the chart empty. Which `.helmignore` filters a dependent chart depends on how the chart is included, see [Charts and dependencies]({{ "/usage/deploy/charts.html#excluding-files-or-directories-from-the-chart" | true_relative_url }}).
