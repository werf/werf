...
- `Fixes`/`Closes` keywords live in the description — GitHub does not auto-close from a comment. A reference to `werf/nelm`, `werf/3p-helm`, `werf/kubedog` or `werf/common-go` does not auto-close at all: link it, and put closing it by hand in the comment's *Follow-up*. When a known commit introduced the bug, pin it: `Fixes: <12+ chars of sha> ("the commit subject")`.
