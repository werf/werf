...
func NewDockerImagesRepo(projectName, imagesRepoAddress, imagesRepoMode string, options DockerImagesRepoOptions) (ImagesRepo, error) {
	...
	logboek.Debug.LogLn("Using images docker registry implementation:", resolvedImplementation)

	resolvedImagesRepoMode, err := docker_registry.ResolveRepoMode(resolvedImplementation, imagesRepoMode)
	if err != nil {
		return nil, err
	}
	logboek.Debug.LogLn("Using images repo mode:", resolvedImagesRepoMode)
