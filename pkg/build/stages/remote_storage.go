...
func (s *RemoteStorage) copyAllFromRemote(ctx context.Context, fromRemote *RemoteStorage, opts copyToOptions) error {
	stageIds, err := fromRemote.StorageManager.GetStagesStorage().GetStagesIDs(ctx, opts.ProjectName)
	if err != nil {
	...
	for _, stageId := range stageIds {
		...
		stageDesc, err := fromRemote.StorageManager.GetStagesStorage().GetStageDesc(ctx, opts.ProjectName, stageId)
		if err != nil {
