...
func (i *DependencyImport) validate() error {
	switch i.Type {
	case ImageNameImport, ImageTagImport, ImageRepoImport, ImageDigestImport:
	default:
		return newDetailedConfigError(fmt.Sprintf("invalid `type: %s` for dependency import, expected one of: %s", i.Type, strings.Join([]string{string(ImageNameImport), string(ImageTagImport), string(ImageRepoImport), string(ImageDigestImport)}, ", ")), i.raw, i.raw.rawDependency.doc())
	}
