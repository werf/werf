...
func (c *GitRemoteExport) validate(disableDeterminism bool) error {
	isDefaultMasterBranch := c.Branch == "" && c.Commit == "" && c.Tag == ""
	isBranch := isDefaultMasterBranch || c.Branch != ""
	if isBranch && !disableDeterminism {
...
If you want to use the branch for remote git mapping instead of commit or tag, then disable werf determinism mode with option --disable-determinsm (or WERF_DISABLE_DETERMINISM=1 env var). However it is NOT RECOMMENDED to use the remote git mapping in a such way. Use a particular unchangeable reference, tag, or commit to provide controllable and predictable lifecycle of software.`
