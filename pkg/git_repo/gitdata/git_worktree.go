...
func GetExistingGitWorktrees(cacheVersionRoot string) ([]*GitWorktreeDesc, error) {
	...
	for _, dir := range []string{filepath.Join(cacheVersionRoot, "local"), filepath.Join(cacheVersionRoot, "remote")} {
		if _, err := os.Stat(dir); os.IsNotExist(err) {
			...
			return nil, fmt.Errorf("error accessing dir %q: %s", dir, err)
		}

		files, err := ioutil.ReadDir(dir)
		if err != nil {
			return nil, fmt.Errorf("error reading dir %q: %s", dir, err)
		}
		...
		for _, finfo := range files {
			...
			if !finfo.IsDir() {
				if err := os.RemoveAll(worktreePath); err != nil {
					return nil, fmt.Errorf("unable to remove %q: %s", worktreePath, err)
				}
			}

			size, err := volumeutils.DirSizeBytes(worktreePath)
			if err != nil {
				return nil, fmt.Errorf("error getting dir %q size: %s", worktreePath, err)
			}
