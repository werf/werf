...
var _ = Describe("docker system", func() {
	Describe("readDaemonConfigFromFile", func() {
		...
		It("should return empty config when daemon.json is empty object", func() {
			...
			Expect(os.MkdirAll(dockerDir, 0o755)).To(Succeed())
			Expect(os.WriteFile(filepath.Join(dockerDir, "daemon.json"), []byte("{}"), 0o644)).To(Succeed())
		...
		It("should read valid daemon.json", func() {
			...
			Expect(os.WriteFile(filepath.Join(dockerDir, "daemon.json"), []byte(configContent), 0o644)).To(Succeed())
		...
		It("should return error for invalid JSON", func() {
			dockerDir := filepath.Join(tmpDir, ".docker")
			Expect(os.MkdirAll(dockerDir, 0o755)).To(Succeed())
			Expect(os.WriteFile(filepath.Join(dockerDir, "daemon.json"), []byte("not valid json"), 0o644)).To(Succeed())
