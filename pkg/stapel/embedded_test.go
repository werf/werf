...
func TestNormalizeEmbeddedPlatform(t *testing.T) {
	tests := []struct {
		name           string
...
func TestDecompressAndVerify_valid(t *testing.T) {
...
func TestDecompressAndVerify_mismatch(t *testing.T) {
	payload := []byte("stapel tar payload")
...
func TestDecompressAndVerify_invalidGzip(t *testing.T) {
	_, err := decompressAndVerify([]byte("not gzip data"), "deadbeef")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "gzip")
}

func TestIsDefaultImageRef(t *testing.T) {
