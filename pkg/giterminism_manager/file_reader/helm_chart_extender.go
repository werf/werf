...
func (r FileReader) LoadChartDir(ctx context.Context, chartDir string) ([]*loader.BufferedFile, error) {
	relDir := r.absolutePathToProjectDirRelativePath(chartDir)
...
func (r FileReader) loadChartDir(ctx context.Context, relDir string) ([]*loader.BufferedFile, error) {
	...
	if err := r.WalkConfigurationFilesWithGlob(
		...
		func(relativeToDirNotResolvedPath string, data []byte, err error) error {
			...
			res = append(res, &loader.BufferedFile{Name: relativeToDirNotResolvedPath, Data: data})
