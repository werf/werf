...
*   В репозитории мы добавили ключ к регистри, чтобы кубы могли спулить наши обрпзы, и запустить приложение
...
```bash
user:~/tools$ werf helm secret generate-secret-key
ad747845284fea7135dca84bde9cff8e
user:~/tools$ export WERF_SECRET_KEY=ad747845284fea7135dca84bde9cff8e
```
...
```bash
user:~/tools$ werf helm secret values edit ./helm/secret-values.yaml
```
...
## Оптимизация сборки
А еще, у pip есть кеш, чтобы каэжый раз менеджер зависимостей не скачивал заного один и тот-же пакет.
