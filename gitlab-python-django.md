...
{{TODO: Проверить запуск этой команды локально, и проработать настройки через переменные, чтобы при их отсутствии все работало}}
...
Функция `{{ tuple "django" . | include "werf_container_image"}}` которая подставит необходимый нам образ с приложением.
...
Поэтому в настройках мы изменим некоторые параметры, и будем брать их из переменных, например:
```
DEBUG
SECRET_KEY
```
...
Для примера, мы пробросим значение переменной `DEBUG`.
...
По умолчанию Django не логирует ошибки и критичные логи, а отправляет на email администратора.
Что-бы все логи отправлялись в stdout, его нужно сконфигурировать:
Добавить в файл настроек словарь:
...
При запуске - приложение работает на стандартном порту `80`
...
Для работы нашего приложения потребуется `SECRET_KEY ` с помощью которого происходит шифрование секретов, генерация сессий, и все важные операции.

Все секретные данные - нужно описывать в зашифрованом файле `.helm/secret-values.yaml`
...
app:
  secret_key:
    production: 100083f330adfb9e13fff74c9ab71b93ed77704aca3a0c607679336e099d48977d6565b314c06b8ad7aefc9d8d90629e92d851b573a89915ff036239de129d722ef5
    _default: 100083f330adfb9e13fff74c9ab71b93ed77704aca3a0c607679336e099d48977d6565b314c06b8ad7aefc9d8d90629e92d851b573a89915ff036239de129d722ef5
```
...
{{TODO: ansible поддерживает прямую работу с pip, и мы не пишем это, а вызываем соответствующий модуль, как тут лучше расписать?}}
```yaml
  install:
  - name: Install python requirements
    pip:
...
{{ Тут нужно решить, мы делаем отдельный деплоймент, или новый контейнер в дейплое. Я за первый вариант, так легче разграничивать по ингрессам, и в целом понятней}}
