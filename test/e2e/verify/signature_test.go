...
var _ = Describe("Signature", Label("e2e", "signature", "simple"), func() {
	DescribeTable("should sign and verify image manifest and binaries (inhouse)",
		func(ctx SpecContext, testOpts integrityTestOptions) {
			...
			buildOut, buildReport := werfProject.BuildWithReport(ctx, SuiteData.GetBuildReportPath(buildReportName), &werf.BuildWithReportOptions{CommonOptions: werf.CommonOptions{ExtraArgs: extraBuildArgs}})
