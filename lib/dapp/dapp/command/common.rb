    ...
    module Command
      ...
      module Common
        ...
        def validate_repo_name(repo)
          raise(Error::Dapp, code: :repo_name_incorrect, data: { name: repo }) unless ::Dapp::Dimg::DockerRegistry.repo_name?(repo)
        end
        ...
        def one_dimg!
          raise Error::Dapp, code: :command_unexpected_dimgs_number, data: { dimgs_names: build_configs.map(&:_name).join(' ') } unless build_configs.one?
        end
