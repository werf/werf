module Dapp
  module Dimg
    class Builder::Ansible < Builder::Base
      ...
      def ansible_bin
        dimg.dapp.ruby2go_dappdeps_command(dappdeps_name: :ansible, command: :bin, options: { bin: :ansible })
      end

      def ansible_playbook_bin
        dimg.dapp.ruby2go_dappdeps_command(dappdeps_name: :ansible, command: :bin, options: { bin: :"ansible-playbook" })
      end

      def python_path
        dimg.dapp.ruby2go_dappdeps_command(dappdeps_name: :ansible, command: :bin, options: { bin: :python })
      end
      ...
      def ansible_container
        dimg.dapp.dappdeps_container(:ansible)
      end
