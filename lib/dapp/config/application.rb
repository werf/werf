module Dapp
  module Config
    ...
    class Application
      ...
      def chef
        raise Error::Config, code: :another_builder_defined unless _builder == :chef
        @_chef ||= Chef.new
      end

      def shell
        raise Error::Config, code: :another_builder_defined unless _builder == :shell
        @_shell ||= Shell.new
      end
      ...
      def builder(type)
        raise Error::Config, code: :builder_type_is_not_supported, data: { type: type } unless [:chef, :shell].include?((type = type.to_sym))
