...
* Могут использоваться такие же директивы, как у приложения, но с некоторыми особенностями:
  ...
  * [shell](#shell): добавляется директива [build_artifact](#shellbuild_artifact-image-cache_version-cache_version).
...
* Структура файлов.
  * Атрибуты.
    * Директория attributes — содержит файлы атрибутов.
    * Включенные атрибуты активируются всегда, не зависимо от наличия файла рецепта.
  * Файлы.
...
* Установка [mdapp модуля](#mdapp-модуль).
  * Атрибуты.
    * Файл attributes/\<файл-атрибутов\>.rb устанавливается в attributes/\<файл-атрибутов\>.rb.
      * Для каждого включенного файла атрибутов при его наличии.
  * Файлы.
...
* Обязателен хотя бы один из параметров **before** или **after** с одним из поддерживаемых значений:
  * параметр определяет порядок применения артефакта (до или после);
  * значение определяет стадию (стадиии install или setup).
...
#### chef.attribute \<attr\>[, \<attr\>, \<attr\> ...]
Включить переданные имена файлов атрибутов [модулей](#mdapp-модуль) для chef builder в данном контексте.

Подробнее см.: [mdapp модуль](#mdapp-модуль) и [установка стадии cookbook\`а](#Установка-стадии-cookbookа).

#### chef.remove_attribute \<attr\>[, \<attr\>, \<attr\> ...]
Выключить переданные имена файлов атрибутов для chef builder в данном контексте.

#### chef.reset_attributes
Выключить все файлы атрибутов [модулей](#mdapp-модуль) для chef builder в данном контексте.

##### chef.reset_all
Выключить все рецепты из [приложения](#cookbook-приложения), все модули для chef builder и все атрибуты [модулей](#mdapp-модуль) в данном контексте.
