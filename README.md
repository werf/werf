...
#### Проект (project)
Проект — это директория, содержащая приложение или набор приложений (см. [приложение](#Приложение)).
...
#### Директория проекта
Директория проекта — это директория, в которой находится директория .dapps или, при отсутствии .dapps — это директория, содержащая Dappfile.
...
#### Тип сборки
Dapp поддерживает 2 типа сборки: [chef проект](#Chef-проект) и [shell проект](#Shell-проект).
* В одном [приложении](#Приложение) может быть использован лишь один тип сборки.
* Тип сборки определяется автоматически при первом использовании соответствующей конструкции в [Dappfile](#Dappfile).

#### Chef проект
Chef проект — это проект, для сборки которого используются [chef cookbook проекта](#Chef-cookbook-проекта) и [mdapp модули](#Mdapp-модуль).

#### Chef cookbook проекта
Chef cookbook проекта — это основной chef cookbook, связанный с одним [приложением](#Приложение).
* Обязательные файлы:
  * metadata.rb
  ...
  * Berksfile
    * Содержит все зависимости, в т.ч. [mdapp модули](#mdapp-модуль).
  * Berksfile.lock
    * Может отсутствовать в [dev режиме](#dev-режим).

#### Mdapp модуль

#### Cookbook стадии

#### Установка cookbooks стадии

#### Контрольная сумма cookbook стадии

#### Дерево chef cookbooks
Дерево chef cookbooks — это результат выполнения berks vendor в chef проекте

#### Shell проект
Shell проект — это проект, для сборки которого используются команды bash, заданные в [Dappfile](#Dappfile).

#### Build директория
Build директория — это директория для хранения кэша и производных данных сборок.
* Используется для:
  * Кэширования дерева chef cookbook`ов (результат berks vendor) между сборками.
    * Этот результат при возможности кэшируется между сборками.
  * Кэширования [контрольных сумм cookbook`ов](#Контрольная-сумма-cookbook-стадии) каждой из стадий.
  * Хранения файлов блокировок проекта.
* Путь к build директории.
  * По умолчанию: \<[корень проекта]\>/.dapps_build.
...
* Одна и та же build директория будут пользуется всеми [приложениями](#Приложение), описанными в данном [проекте](#Проект).
...
* Используется для хранения:
  ...
  * Файлов chef cookbook`ов, связанных со стадией, во время выполнения сборки стадии.
* Путь к tmp директории: /tmp/dapp-\<date\>-\<random\>.
