...
* Обязательные файлы:
  ...
  * Berksfile
    * Содержит все зависимости, в т.ч. [mdapp модули](#mdapp-модуль).
  * Berksfile.lock
    * Может отсутствовать в [dev режиме](#dev-режим) — будет создан во время выполнения сборки.
* Структура файлов.
  * Атрибуты.
...
Установка стадии cookbook`а — это процесс копирования файлов стадии cookbook`а во [временное хранилище](#Tmp-директория), подключаемое в дальнейшем в контейнер для сборки стадии.
* Установка cookbook`а приложения.
  ...
  * Рецепты.
    ...
    * При отсутствии рецептов генерируется пустой рецепт recipes/void.rb
      * Отсутствие рецептов подразумевает:
        * отсуствие включенных рецептов в [Dappfile](#Dappfile)
        * отсутствие файлов рецептов (recipes/\<стадия\>/\<рецепт\>.rb) для всех включенных рецептов.
      * Это позволяет активировать атрибуты, объявленные в данном cookbook`е.

#### Контрольная сумма стадии cookbook`а
Контрольная сумма стадии cookbook`а — это 
...
* Используется для:
  * Кэширования [дерева cookbooks](#Дерево-cookbooks) между сборками.
    * Этот результат при возможности кэшируется между сборками.
  * Кэширования [контрольных сумм cookbook`ов](#Контрольная-сумма-cookbookа-стадии) каждой из стадий.
...
* Используется для хранения:
  ...
  * Файлов cookbook`ов, связанных со стадией, во время выполнения сборки стадии.
  * [Стадии cookbook`а](#Стадия-cookbookа), [установленной](#Установка-стадии-cookbookа) во время сборки стадии.
