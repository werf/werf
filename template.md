...
**_configVersion _**- в данном случае определяет версию синтаксиса используемую в werf.yaml.
...
```
	...
	Werf подразумевает что ваша сборка будет происходить внутри директории склонированного git репозитория. Потому мы списком можем указывать директории и файлы относительно корня репозитория которые нам нужно добавить внутрь имиджа.
...
Внутри нашего проекта создаем структуру директорий `.helm -> templates`

Так же добавляем файл `values.yaml`

![alt_text](images/-4.png "image_tooltip")

Директория `.helm` как раз и будет тем местом где мы будем описывать наш деплой.

Директория `templates` будет содержать объекты kubernetes которые запустят наше приложение и сделают его доступным из кластера.

`values.yaml` - как и в Helm это файл с переменными для параметризации шаблонов.
