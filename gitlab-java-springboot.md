...
Для артефакта настроили пересборку npm install в случае изменения packaje.json и webpack.config.js. А так же запуск npm run build при любом изменении файла в репозитории.

### Изменения в деплое

Запустим наш контейнер с nginx в том же поде с приложением но в отдельном контейнере. В deployment допишем:
...
В ingress направим все что связано с ассетами на порт 80:
```yaml
...
      - path: /static/
        backend:
